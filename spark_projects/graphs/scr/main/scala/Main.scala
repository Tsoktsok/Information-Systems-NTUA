...
object Main {
   val HDFSBaseURL = "hdfs://master:54310"
    ...
    def pageRank(spark: SparkSession,graphFilePath: String): (Long, Long, Long, Long, Long, Long) = {
    val sc = spark.sparkContext
    //val graphFilePath1 = s"$HDFSBaseURL//user/user/graph_data/test2.csv"
    ...
    def main(args: Array[String]): Unit = {
        ...
        if (args.length < 2) {
        println("Usage: Main <algorithmToExecute> <graphFilePath>")
        sys.exit(1) // Exit the program
        }

        // Parse input args
        val algorithmToExecute = args(0)
        val graphFilePath = args(1).toString

        val outputFolder = "spark_graph_results"

        // Create a spark session and setup the Stage Metrics
        val spark = SparkSession    
        .builder
        .appName(s"$algorithmToExecute-$graphFilePath")
        ...
        if (algorithmToExecute == "pageRank") {
            graphInfo = pageRank(spark, graphFilePath)
            }
        }
        ...
        testDF.coalesce(1).write.mode("append").option("header", "true").csv(outputFolder)
